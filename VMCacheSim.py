    ...
    while any(not p["done"] for p in processes):
        for proc in processes:
            ...
            while instructions_processed < n:
                ...
                # Skip blank lines or lines that aren't the start of an instruction
                line = line.strip()
                ...
                # Extract addresses and handle memory accesses
                instr_addr = int(line1[10:18], 16)
                handle_memory_access(instr_addr, proc, phys_mem, processes)

                # Check dstM and srcM (4 if valid)
                dst_addr_str = line2[6:14]
                if dst_addr_str != "00000000" and "--------" not in line2[15:23]:
                    handle_memory_access(
                        int(dst_addr_str, 16), proc, phys_mem, processes
                    )

                src_addr_str = line2[33:41]
                if src_addr_str != "00000000" and "--------" not in line2[42:50]:
                    handle_memory_access(
                        int(src_addr_str, 16), proc, phys_mem, processes
                    )

                instructions_processed += 1

    # Final Page Fault Summary
    print("\n***** Simulation Results *****")
    for i, p in enumerate(processes):
        print(f"Process {i+1} Page Faults: {p['page_faults']}")
